...
def sync_strava_activities(athlete_id: str = "viet", *, max_pages: int = 2) -> dict[str, object]:
    ...
    from app.strava_details import enrich_runs
    detail_status = enrich_runs(athlete_id, headers)
...
def _enrich_pending_effort(athlete_id, headers):
    # Compatibility for callers of the earlier, incorrectly addressed lookup.
    from app.strava_details import enrich_runs
    return enrich_runs(athlete_id, headers)
