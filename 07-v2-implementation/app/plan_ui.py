...
<section id="plan" class="panel">
 ...
 <div class="field"><label for="raceCity">Race city / location</label><input id="raceCity" maxlength="160" placeholder="e.g. Taipei"></div>
 <div class="field"><button id="raceFind" type="button" class="secondary">Find race location</button><label for="racePlace">Matching location</label><select id="racePlace"><option value="">Location not set</option></select><span id="raceLocationStatus" class="hint"></span></div>
 <div class="field"><label for="raceStart">Race start (local time at race)</label><input id="raceStart" type="time"><span class="hint">Used for weather during the race. Location and date are sent to Open-Meteo.</span></div>
...
<script>
(()=>{
 const $=id=>document.getElementById(id);let progress=null,places=[],racePlaces=[],raceLocation=null;
 ...
 async function loadPlan(){progress=await call('/app/api/plan');const g=progress.goal;if(g){$('raceName').value=g.name;raceLocation=g.location||null;$('raceCity').value=raceLocation?.name||'';$('raceStart').value=(g.start_time||'').slice(0,5);$('racePlace').replaceChildren();const locationOption=text($('racePlace'),'option',raceLocation?.name||'Location not set');locationOption.value='';$('raceLocationStatus').textContent=raceLocation?`Race time zone: ${raceLocation.timezone}`:'';$('raceDate').value=g.race_date;$('raceDistance').value=g.distance_km;$('raceTime').value=String(Math.floor(g.goal_minutes/60)).padStart(2,'0')+':'+String(Math.round(g.goal_minutes%60)).padStart(2,'0');$('planTimezone').value=g.timezone;const p=progress.goal_pace_seconds_km;$('goalSummary').textContent=`${g.name} · ${g.race_date} · ${$('raceTime').value} goal · ${Math.floor(p/60)}:${String(p%60).padStart(2,'0')}/km target pace · ${progress.days_to_race} days to race`;fillGoalDays();}
 ...
 $('goalForm').addEventListener('submit',async e=>{e.preventDefault();try{if($('raceCity').value.trim()&&!raceLocation)throw new Error('Find and select the race location, or clear the city to add it later.');const parts=$('raceTime').value.split(':').map(Number);await send('/app/api/goal','PUT',{name:$('raceName').value.trim(),race_date:$('raceDate').value,distance_km:Number($('raceDistance').value),goal_minutes:parts[0]*60+parts[1],timezone:$('planTimezone').value.trim(),new_race:$('newRace').checked,location:raceLocation,start_time:$('raceStart').value||null});$('newRace').checked=false;$('goalStatus').textContent='Race goal saved.';await loadPlan();window.dispatchEvent(new Event('strideai:goal-saved'))}catch(err){$('goalStatus').textContent=err.message}});
 $('raceCity').addEventListener('input',()=>{raceLocation=null;racePlaces=[];$('racePlace').replaceChildren();text($('racePlace'),'option','Find and select a matching location').value='';$('raceLocationStatus').textContent='';});
 $('raceFind').onclick=async()=>{try{racePlaces=await call('/app/api/weather/places?name='+encodeURIComponent($('raceCity').value.trim()));$('racePlace').replaceChildren();text($('racePlace'),'option','Choose a location').value='';racePlaces.forEach((p,i)=>text($('racePlace'),'option',p.name).value=String(i));$('raceLocationStatus').textContent=racePlaces.length?'Select the matching city.':'No matching city found. Try a nearby city.';}catch(e){$('raceLocationStatus').textContent=e.message;}};
 $('racePlace').onchange=()=>{raceLocation=$('racePlace').value===''?null:racePlaces[Number($('racePlace').value)];$('raceLocationStatus').textContent=raceLocation?`Race time zone: ${raceLocation.timezone}`:'';};
