...
await page.getByText('Forgot your pre-run check-in? (optional)',{exact:true}).click();
await page.getByLabel('How did you feel before running?').selectOption('good');
await page.getByLabel('Sleep hours (optional)').fill('7');
await page.getByRole('button',{name:'Save late check-in',exact:true}).click();
await page.getByRole('status').filter({hasText:'Late check-in saved at'}).waitFor();
await page.getByRole('button',{name:'Test AI connection',exact:true}).click();
await page.getByRole('status').filter({hasText:'AI test did not succeed'}).waitFor();
const note=await api(`/app/api/coach/workouts/${rows[0].id}/late-checkin`,undefined,'GET');
if(note.checkin.feeling!=='good'||note.timing!=='retrospective')throw Error('Late note not saved');
if(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth))throw Error('Mobile overflow');
if(errors.length)throw Error(errors.join('; '));console.log('Late check-in and disabled-provider diagnostic passed');await browser.close();
